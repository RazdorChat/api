...
def user_friend_add(request, thread_id):
	...
	if not checks.authenticated(auth, id_generator.get_session_token(request.app.ctx.redis, requester)): # The client is trying to send a request without auth.
...
def user_authkey_id(request, user_id):
	...
	check = request.app.ctx.hasher.verify_password_hash(_json['auth'], data['authentication'], data['salt']) # Verify their password is correct.

	if check != True: # the hash doesnt match
		return json({"op": ops.Unauthorized.op}, status=401)
	
	elif check == True: # the hash matches
		key = id_generator.generate_session_token(request.app.ctx.redis, user)
