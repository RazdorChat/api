...
def message_delete(request, thread_type, thread_id, message_id):
	...
	if not checks.authenticated(_json["auth"], id_generator.get_session_token(request.app.ctx.redis, _json['requester'])): # Client is trying to delete a message as a user they are not.
...
async def message_send(request, thread_type, thread_id):
	...
	if not checks.authenticated(request.json["auth"], id_generator.get_session_token(request.app.ctx.redis, data['requester'])): # Client is trying to send a message as a user they are not, or their auth is wrong.
		return json({"op": ops.Unauthorized.op}, status=401)
	...
	await request.app.ctx.sse.register_event(events.Event("new_message", int(data['requester']), thread_id, thread_type,
		{
...
def message_mass_get(request, thread_type, thread_id):
	...
	if not checks.authenticated(request.json["auth"], id_generator.get_session_token(request.app.ctx.redis, request.json["requester"])): # Client is trying to send a message as a user they are not, or their auth is wrong.
		return json({"op": ops.Unauthorized.op}, status=401)
