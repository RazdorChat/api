...
class UserAPI:
    ...
    def user_get(self, request, thread_id):
        db = request.ctx.db
    ...
    @openapi.response(400, {"application/json" : ops.MissingRequiredJson})
    @openapi.response(401, {"application/json" : ops.Unauthorized})
    def user_friend_add(self, request, thread_id):
        db = request.ctx.db
        ...
        if int(self, requester) == thread_id:
            return json({"op": "Error. Not lonely enough to send friend requests to self"}) # we do a little trolling

        if not checks.authenticated(auth, id_generator.get_session_token(self, request.ctx.redis, requester)): # The client is trying to send a request without auth.
    ...
    @openapi.response(404, {"application/json" : ops.Void})
    @openapi.response(401, {"application/json" : ops.Unauthorized})
    def user_relationships(self, request, thread_id):
        db = request.ctx.db
        ...
        if not checks.authenticated(auth, id_generator.get_session_token(self, request.ctx.redis, user)):
    ...
    @openapi.response(404, {"application/json" : ops.Void})
    @openapi.response(401, {"application/json" : ops.Unauthorized})
    def user_friend_accept(self, request):
        db = request.ctx.db
        ...
        if not checks.authenticated(auth, id_generator.get_session_token(self, request.ctx.redis, parent)):
            return json({"op": ops.Unauthorized.op}, status=401)
    ...
    @openapi.response(400, {"application/json" : ops.MissingRequiredJson})
    @openapi.response(401, {"application/json" : ops.Unauthorized})
    def user_delete(self, request, thread_id):
        db = request.ctx.db
        user = thread_id # For readability 
        ...
        if not checks.authenticated(data["auth"], id_generator.get_session_token(self, request.ctx.redis, user)):
            return json({"op": ops.Unauthorized.op}, status=401)
    ...
    @openapi.response(404, {"application/json" : ops.Void})
    @openapi.response(401, {"application/json" : ops.Unauthorized})
    def user_authkey_id(self, request, user_id):
        db = request.ctx.db
        ...
        if check != True: # the hash doesnt match
            ...
            key = id_generator.generate_session_token(self, request.ctx.redis, user)
    ...
    @openapi.response(404, {"application/json" : ops.Void})
    @openapi.response(401, {"application/json" : ops.Unauthorized})
    def user_authkey(self, request, username, discriminator):
        db = request.ctx.db
        _json = request.json
        if not _json:
            return json({"op": ops.MissingJson.op})
